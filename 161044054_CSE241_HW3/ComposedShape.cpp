	...
	for(k=nOfVertCircle;k>0;k--){
		...
		for(l=1;l<m;l++,plus=plus+2*In.getRadius()){
			...
			for(rotate=0;rotate<360;rotate=rotate+60){
				//generate the one small shape and saves the vector
				circle smallC(In.getRadius(),'i',x+plus,y,rotate,0,0);//container.radius'a göre rotate var
				circ.push_back(smallC);
			}
		}
		// updates
...
void ComposedShape::optimalFit(circle Co,triangle In){
	...
	for(k=nOfTri;k>0;k--){
		...
		for(l=0;l<m;l++,plus=plus+In.getLateral()){
			...
			for(rotate=0;rotate<360;rotate=rotate+60){						
				//generate the one small shape and saves the vector
				triangle smallTri(In.getLateral(),'i',x+plus,y,x+In.getLateral()/2.0+plus,y+(In.getLateral()*SQR3DIV2),(x-In.getLateral()/2.0)+plus,y+(In.getLateral()*SQR3DIV2),rotate,0,0);
			...
			if(k!=1){
				...
				for(rotate2=0;rotate2<360;rotate2=rotate2+60){	
					//generate the one small shape and saves the vector
					triangle reverseSmallTri(In.getLateral(),'i',x+plus,y+(2*In.getLateral()*SQR3DIV2),x+In.getLateral()/2.0+plus,y+(In.getLateral()*SQR3DIV2),(x-In.getLateral()/2.0)+plus,y+(In.getLateral()*SQR3DIV2),rotate2,0,0);
...
double ComposedShape::ShapeElem::getPerimeter(){
	if(shape==rectangle){
		class rectangle *pRec = static_cast<class rectangle*>(shapeObject);
		return pRec->periLength();
	}
	else if(shape==circle){
		class circle *pCirc = static_cast<class circle*>(shapeObject);
		return pCirc->periLength();
	}
	else if(shape==triangle){
		class triangle *pTri = static_cast<class triangle*>(shapeObject);
...
ostream& operator<<(ostream& outputStream,const ComposedShape::ShapeElem& Shape){
	if(Shape.shape==ComposedShape::ShapeElem::rectangle){
		class rectangle *pRec = static_cast<class rectangle*>(Shape.shapeObject);
		outputStream<<*pRec;
	}
	else if(Shape.shape==ComposedShape::ShapeElem::triangle){
		class triangle *pTri = static_cast<class triangle*>(Shape.shapeObject);
		outputStream<<*pTri;
	}
	else if(Shape.shape==ComposedShape::ShapeElem::circle){
		class circle *pCirc = static_cast<class circle*>(Shape.shapeObject);
		outputStream<<*pCirc;
	}
	return outputStream;
}
//I add the new composedShape variable from old one
ComposedShape& ComposedShape::operator+=(const ComposedShape& addShape){
	this->triIn=addShape.getTriIn();
	this->triCo=addShape.getTriCo();
	this->circIn=addShape.getCircIn();
	this->circCo=addShape.getCircCo();
	this->recIn=addShape.getRecIn();
	this->recCo=addShape.getRecCo();
	this->innerShapeVec=addShape.innerShapeVec;
    return *this;
}
